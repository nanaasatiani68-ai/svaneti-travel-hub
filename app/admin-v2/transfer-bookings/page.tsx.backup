                      ...
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          ...
                          <div className="mt-4 grid gap-2 text-white/80 sm:grid-cols-2">
                            <p>💰 {formatTransferPrice(transfer)}</p>
...
function formatTransferPrice(transfer: Transfer) {
  const type = normalizePriceType(transfer.price_type);
  ...
  const formatted = Number(transfer.price).toLocaleString("ka-GE");

  if (type === "from") {
    return `${formatted} ₾-დან`;
  }

  return `${formatted} ₾ მანქანაზე`;
}

function normalizePriceType(value: string | null | undefined): PriceType {
  if (value === "negotiable" || value === "from") {
    return value;
  }
